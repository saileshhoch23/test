  ...
  const getData = async () => {
    try {
      ...
      const responseData = await postJson(
        "office/get_offices_canteen_orders_list",
        payload
      );
        ...
        <div className="row mb-4">
          {canteenPymentlist?.length > 0 &&
            canteenPymentlist?.map((item, index) => (
              <div
                className="col-3 col-lg-3 col-md-4 col-sm-12 mb-2"
                key={index}
              >
                <div className="card add-tocart payment-card mt-3">
                  <div
                    className="card-body"
                    onClick={() => {
                      navigate(`/transaction/${item?.canteenId}`);
