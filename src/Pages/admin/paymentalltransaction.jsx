...
const Paymentalltransaction = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'isPaymentSuccess',
      render: (isPaymentSuccess) => (
        <span
          style={{ cursor: "pointer" }}
          className={`${isPaymentSuccess ? "bg-green" : "bg-red "}`}
        >
          {isPaymentSuccess === true ? "Paymeny Success" : "Payment filed "}
        </span>
      ),
    },
    {
  ...
  const dataSource = pymentTraction.map((item) => ({
    ...
    amount: item?.amount,
    paymentMethod: item?.paymentMethod.toLowerCase() ==="cash" ? "Cash" :"Online",
    isPaymentSuccess: item?.isPaymentSuccess ,
  ...
  return (
    <>
      ...
      <Modal
        ...
        onCancel={() =>{ setSelectedOrder(null); setOrderDetails({}) }}
        onOk={() => { setSelectedOrder(null); setOrderDetails({}) }}
        footer={null}
      >
          {loadingDetails ? (
        ...
        <div className="mt-5 add-order d-flex justify-content-between align-item-center px-3 py-3">
          ...
          <div className="">
            <button
              ...
              type="button"
              onClick={() => {
                 setSelectedOrder(null); setOrderDetails({}) 
...
export default Paymentalltransaction;
