...
import Canteenuseredit from './editamodel/canteenuseredit';
const Canteenuser = () => {
    ...
    const [canteenedit, setCanteenEdit] = useState(false);
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, record) => (
          ...
          onClick={() =>{setCanteenEdit(true); setEditeRecorde(record);} }
          icon={<EditOutlined />}
        ></Button>
        ...
        try {
          const payload = {
            ...
            pageable: {
              pageno:page,
             pagesize:pagesize,
             filtertext:searchValue?`name like '%${searchValue}%' OR user_id like '%${searchValue}%' OR first_name like '%${searchValue}%'  OR last_name like '%${searchValue}%' OR email like '%${searchValue}%' OR start_time like  '%${searchValue}%' OR end_time like '%${searchValue}%' OR address1 like '%${searchValue}%' OR canteen_building_name like '%${searchValue}%'`:""
        }
          }
          setLoading(true);
  ...
  return (
    ...
    <Layout className="bg-white p-5">
      ...
      <Canteenuseredit onClose={()=>setCanteenEdit(false)} canteenedit={canteenedit} record={editerecord}  onClear={()=> setEditeRecorde({})}/>
      </>
  )
}
