...
import Officeuseredit from './editamodel/officeuseredit';
const Officeuser = () => {
    ...
    const [officedit, setOfficeEdit] = useState(false);
    ...
    const columns = [
        ...
        },
        {
          ...
          render: (text, record) => (
            ...
            onClick={() =>{setOfficeEdit(true); setEditeRecorde(record);}}
            icon={<EditOutlined />}
          ></Button>
        ...
        try {
          const payload = {
            ...
            pageable: {
              pageno:page,
          pagesize:pagesize,
          filtertext:searchValue?`user_id like '%${searchValue}%' OR first_name like '%${searchValue}%'  OR last_name like '%${searchValue}%' OR email like '%${searchValue}%' OR office_name like '%${searchValue}%' OR office like '%${searchValue}' OR address1 like '%${searchValue}%' OR canteen_building_name like '%${searchValue}%'`:""
        }
          }
          setLoading(true);
  ...
  return (
    ...
    <Layout className="bg-white p-5">
      ...
      <Officeuseredit onClose={()=>setOfficeEdit(false)} officedit={officedit} record={editerecord}  onClear={()=> setEditeRecorde({})} />
      </>
  )
}
