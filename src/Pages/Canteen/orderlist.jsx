import { Layout, Space,DatePicker,Input, Spin } from "antd";
...
import Otpvrfcation from "./otpvrfcation";
...
const Orderlist = () => {
 ...
 const [submitrejectloader, setSubitrejectloader] = useState(false)
  useEffect(()=>{
    getData()
  },[page, pageSize,  searchValue, selectedDates])
  const getData = async () => {
    try {
      const payload = {
      ...
      const responseData = await postJson(
        "order/get_all_order_list_page",payload
      );
  ...
  const orderAccept = async (item,) => {
    try {
      setSubmitLoader(true);
      const responseData = await postJson("order/accept_order", {
        orderId: item?.orderId,
  ...
  return (
    ...
    <Layout className="bg-white p-5 h-90">
      ...
      <div className="row">
        ...
        {orderDetils?.orderId ? (
          <div className="col-lg-8">
                  ...
                  src={
                    orderDetils?.officeOwnerDto?.companyLogo !== null
                      ...
                      : "./assets/images/user-img.png"
                        ...
                        alt="img-canteen"
                      />
                      <button className="btn office-edit-btn" type="button" onClick={()=>setEditModel(true)}><TbEdit /></button>
                      </div>
                      <div>
            ...
            {  orderDetils.isAccepted === "N" && orderDetils?.canceled == false ?  <> <button className=" btn btn btn-outline-danger  me-sm-4 me-2"  type="button" onClick={()=>cancelOrder(orderDetils)}>
            { submitrejectloader?  <Spinner animation="border" size="sm" /> :"Reject"} 
                </button>
                   <button className="btn btn-dark" type="button" onClick={()=>orderAccept(orderDetils)}>
                  { submitloader?  <Spinner animation="border" size="sm" /> :"Accept"}
                  </button>
                  </> 
                :
                orderDetils.isAccepted === "Y" && orderDetils?.canceled == true ?
                <button className="btn btn-dark" type="button" onClick={()=>setOtpModel(true)}>
                Otp
                </button> :
                <button className=" btn btn btn-outline-danger  me-sm-4 me-2"  type="button" onClick={()=>cancelOrder(orderDetils)} disabled={true}>
                 Reject
                    </button>
                }
              </div>
      ...
      <Otpvrfcation otpList={orderDetils} otpModel={otpModel} onClose={()=>setOtpModel(false)} getData={()=>getData()}/>
      <Editeorder editModel={editModel} editeData={orderDetils} onClose={()=>setEditModel(false)} getData={()=>getData()}/>
      </>
  );
};
